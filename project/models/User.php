...
function submitLostAndFoundData($title, $description, $status, $image) {
    ...
    $image_folder = '../views/uploads/' . $image;  // Updated path
...
// Insert post data into the database with the created time
    ...
    $stmt = mysqli_prepare($conn, $sql);
    if ($stmt === false) {
        error_log('SQL preparation failed: ' . mysqli_error($conn)); // Log preparation error
    ...
    if ($result === false) {
        error_log('SQL execution failed: ' . mysqli_error($conn)); // Log execution error
